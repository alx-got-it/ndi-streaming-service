...
def main():
    if not ndi.initialize():
        print("Не удалось запустить NDI.")
        return 0
    ...
    recv_create_desc.ndi_recv_name = "Аудио конвертер"
    ...
    while True:
        ...
        if t == ndi.FRAME_TYPE_VIDEO:
            print("Получены видео данные (%dx%d)." % (v.xres, v.yres))
            ndi.recv_free_video_v2(ndi_recv, v)
            continue

        if t == ndi.FRAME_TYPE_AUDIO:
            print("Получены аудио данные (%d семплов)." % a.no_samples)
            ...
            ndi.util_audio_to_interleaved_16s_v2(a, audio_frame_16bpp_interleaved)
            ndi.recv_free_audio_v2(ndi_recv, a)
            continue

        if t == ndi.FRAME_TYPE_METADATA:
            print("Получены мета-данные.")
            ndi.recv_free_metadata(ndi_recv, m)
            continue

        if t == ndi.FRANE_TYPE_STATUS_CHANGE:
            print("Изменен статус входящего соединения.")
            continue
